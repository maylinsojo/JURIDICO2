...
class m_wds_transportistas extends CI_Model
{
	...
	*/
	...
	/*
		Descripción : Método que lista la información de la transportista
		Parametros  : Nombre => id_transp, Tipo => Int, Obligatorio => Si
		Retorna     : array(
		                    ID                => Int,
							NOMBRE            => Varchar,
	...
	*/
	public function info_transportista($datos){
	...
	}//Fin del método info_transportista
	/**********************************/
	
	/*
		Descripción : Método que lista los estatus de  la transportista
		Parametros  : Ninguno.
		Retorna     : Array( 
		                     ID          => Int, 
		                     DESCRIPCION => Varchar
	...
	*/
	public function estatus_transportista(){
		
		//Query para el estatus de la transportista
		 $sql = "SELECT valor,
		               descripcion
		        FROM estatus
				WHERE tabla = 'transportista'
	...
	}//Fin del método estatus_transportista
	/*************************************/
	...
	*/
	public function editar_transportista($datos){
		
		//Parametros para insertar los cambios en los datos de la transportista
		$parametros = array(
		...
		$this->db->where('id', $datos['id_transp']);
		
		//Evaluamos si se creo la transportista
		if($this->db->update('transportista', $parametros)){
			
			$this->db->trans_commit();
			
			$respuesta = array(
							   'CODIGO_RESPUESTA'  => 1, 
							   'MENSAJE_RESPUESTA' => 'transportista actualizada!'
							  );
			...
			$respuesta = array(
							   'CODIGO_RESPUESTA'  => 0, 
							   'MENSAJE_RESPUESTA' => 'No se pudo actualizar la transportista!'
							  );
	...
	*/
	public function eliminar_transportista($datos){
		
		$this->db->where('id', $datos['id_transp']);
		
		//Evaluamos si se elimina
        if($this->db->delete('transportista')){
			
			$respuesta = array(
							   'CODIGO_RESPUESTA'  => 1, 
							   'MENSAJE_RESPUESTA' => 'Transportista Eliminada!'
							  );
