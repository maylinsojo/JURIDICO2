...
class m_wds_pedagogico extends CI_Model
{
	...
	*/
	public function cursosAsociados($datos){
	...
	*/
	...
	}//Fin del método producto_servicio
	/*********************************/
		...
		if(intval($resultado['despachado']) == 0){
		  ...
		  $this->db->trans_begin();
			...
			$datos = array(
							...
							'coste_imprenta'     => $parametros['coste_imprenta'],
							'id_mod_peda'        => $parametros['productoServ'],
							'id_imprenta'        => $parametros['imprenta'],
							'id_estatus'         => 1
						  );
			...
			if($this->db->insert('despacho_pedagogico', $datos)){
				...
				foreach($parametros['arrayItems'] as $item){
					...
					$datos[] = array(
										  ...
										  'id_modulo_peda'   => $valores->materiales,
											'id_curso'         => $valores->curso,
					...
					$datosAlum = array('num_alumnos' => $valores->eval);
					$id_curso = $valores->curso;

					$this->db->where('id_establecimiento', $parametros['establecimiento']);
					$this->db->where('id_curso', $id_curso);

 					//Actualizamos la cantidad de alumnos
 					$this->db->update('curso_esteblecimiento', $datosAlum);
			  ...
			  if($this->db->insert_batch('detalle_despacho_pedagogico', $datos)){
					...
					$paramImpr = array('precio' => $parametros['coste_imprenta']);
					$this->db->where('id', $parametros['imprenta']);

					//Evaluamos si se creo el usuario
					if($this->db->update('imprenta', $paramImpr)){
