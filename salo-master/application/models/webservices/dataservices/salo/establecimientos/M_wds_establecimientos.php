...
class m_wds_establecimientos extends CI_Model
{
	...
	*/
	public function servicio_establecimiento($datos){
		
		//Query
		$sql = "SELECT m.id,
		               m.descripcion					 
		        FROM servicio m,
				     servicio_establecimiento mu
				WHERE m.id = mu.id_servicio
				AND mu.id_establecimiento = ".$datos['id_ee']."
                AND m.id_estatus = 1
	...
	*/
	public function asociar_servicio_establecimiento($datos){
		
		//Iniciamos una transaccion
		$this->db->trans_begin();
		
		//Eliminamos los servicios anteriores
		$this->db->where('id_establecimiento', $datos['id_ee']);
		
		//Evaluamos
		if($this->db->delete('servicio_establecimiento')){
			
			//Evaluamos
			if($datos['servicios'] == '-1'){
				...
				$numservicios = count($datos['servicios']);
				...
				if($numservicios > 0){
					...
					for($i = 0; $i < $numservicios; $i++){
						...
						$servicios[$i] = array(
										 'id_servicio'        => $datos['servicios'][$i],
										 'id_establecimiento' => $datos['id_ee']
										);
						
					}//Fin del for
					
					if($this->db->insert_batch('servicio_establecimiento', $servicios)){
